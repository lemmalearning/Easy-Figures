...
class Arrow:
	matplotlib_obj = None
	def __init__(self, start, end, color, headWidth, headLength, lw, mplprops, clip, figure):
		# x0,y0 start
		# x1,y1 end
		...
		self.head = self.figure.addPolygon([self.figure.px2unit_c(l_1).tolist()[0], self.figure.px2unit_c(end).tolist()[0], self.figure.px2unit_c(l_2).tolist()[0]], lw=0, mplprops={'color':color}, clip=clip)
		self.line = self.figure.addLine(self.figure.px2unit_c(start).tolist()[0], self.figure.px2unit_c(p_t0).tolist()[0], lw=lw, color=color, clip=clip)
