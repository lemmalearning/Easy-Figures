...
class Arrow:
	...
	def __init__(self, start, end, headWidth, headLength, lw, mplprops, figure):
		...
		self.head = self.figure.addPolygon([self.figure.px2unit_c(l_1).tolist()[0], self.figure.px2unit_c(end).tolist()[0], self.figure.px2unit_c(l_2).tolist()[0]], mplprops={'color': 'k', 'fc':'k'})
		self.line = self.figure.addLine(self.figure.px2unit_c(start).tolist()[0], self.figure.px2unit_c(p_t0).tolist()[0], lw=lw)
